<script setup lang="ts">
   ...
   onMounted(async () => {
      ...
      if (bodyEl && !agentId.value) {
         bodyEl.innerHTML = 'Configurar a docId do agente'
         return
      }

      const res = await getAgentFirestore(agentId.value)
      if (!res) return
      const agentParsed = agentSchema.parse(res)
      if (res) agentActive.value = agentParsed
   })
</script>
